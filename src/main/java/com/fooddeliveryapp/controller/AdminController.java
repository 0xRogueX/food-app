...
public class AdminController {
    ...
    private void manageCategories() {
        ...
        if (choice == 1) {
            ...
            List<String[]> rows = menuService.getAllCategories().stream()
                    .map(c -> new String[]{String.valueOf(c.getId()), c.getName(), c.isActive() ? "Active" : "Inactive"})
                    .toList();
            TablePrinter.print(new String[]{"ID", "Name", "Status"}, rows);
        } else if (choice == 3) {
            long id = ConsoleInput.getLong("Category ID: ");
    ...
    private void manageMenuItems() {
        ...
        int choice = ConsoleInput.getInt("Choose: ");
        if (choice == 1) {
            long catId = ConsoleInput.getLong("Category ID: ");
            ...
            List<String[]> rows = menuService.getAllMenuItems().stream()
                    .map(m -> new String[]{String.valueOf(m.getId()), m.getName(), String.valueOf(m.getCategoryId()), FormatUtil.formatCurrency(m.getPrice()), m.isAvailable() ? "Yes" : "No"})
                    .toList();
            TablePrinter.print(new String[]{"ID", "Name", "Category ID", "Price", "Available"}, rows);
        } else if (choice == 3) {
            long id = ConsoleInput.getLong("Item ID: ");
    ...
    private void viewUsers(Role role) {
        List<String[]> rows = userService.getUsersByRole(role).stream()
                .map(u -> new String[]{String.valueOf(u.getId()), u.getName(), u.getEmail(), u.getPhone()})
                .toList();
    ...
    private void viewDeliveryAgents() {
        List<String[]> rows = userService.getAllDeliveryAgents().stream()
                .map(a -> new String[]{String.valueOf(a.getId()), a.getName(), a.isAvailable() ? "Yes" : "No", String.valueOf(a.getTotalDeliveries()), String.format("%.1f", a.getRating())})
                .toList();
    ...
    private void viewOrders() {
        List<String[]> rows = orderService.getAllOrders().stream()
                .map(o -> new String[]{
                        ...
                        o.getDeliveryAgentId() != null ? String.valueOf(o.getDeliveryAgentId()) : "Unassigned",
                        o.getStatus().name(),
                        FormatUtil.formatCurrency(o.getFinalAmount())
                })
                .toList();
        TablePrinter.print(new String[]{"Order ID", "Customer ID", "Agent ID", "Status", "Total"}, rows);
    }
    ...
    private void viewFinance() {
        ...
        List<String[]> rows = paymentService.getAllPayments().stream()
                .map(p -> new String[]{String.valueOf(p.getPaymentId()), String.valueOf(p.getOrderId()), p.getMode().name(), p.getStatus().name(), FormatUtil.formatCurrency(p.getAmount())})
                .toList();
